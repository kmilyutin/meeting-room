...
class BookingForm(forms.ModelForm):
        ...
        **kwargs,
        ...
        if allow_room_change:
            self.fields['room'].queryset = (
                self.fields['room']
                .queryset.filter(status='available')
                .order_by('name')
            )
        else:
    ...
    def clean_participants(self):
        ...
        if self.room and participants > self.room.capacity:
            raise forms.ValidationError(
                f'Комната вмещает не более '
                f'{self.room.capacity} участников.',
            )
    ...
    def clean(self):
        ...
        if start_time and end_time:
            ...
            is_within_working_hours = (
                work_start <= start_t <= work_end
                and work_start <= end_t <= work_end
            )

            if not is_within_working_hours:
                raise forms.ValidationError(
                    'Бронирование возможно только '
                    'в рабочее время с 09:00 до 20:00.',
                )
        ...
        if selected_room and selected_room.status != 'available':
            raise forms.ValidationError(
                'Эта комната недоступна для бронирования.',
            )
        ...
        if selected_room and start_time and end_time:
            ...
            if conflicts.exists():
                raise forms.ValidationError(
                    'Комната уже забронирована на выбранное время.',
                )
